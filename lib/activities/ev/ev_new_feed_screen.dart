...
class NewsFeedScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: NotificationListener<ScrollNotification>(
        ...
        child: Obx(() {
          if (controller.isLoading.value && controller.displayNewsList.isEmpty) {
          ...
          if (controller.displayNewsList.isEmpty) {
          ...
          return RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemCount: controller.displayNewsList.length + (controller.hasMore.value ? 1 : 0),
              itemBuilder: (context, index) {
                if (index == controller.displayNewsList.length) {
                  return _buildLoadMoreIndicator(controller);
                }

                final item = controller.displayNewsList[index];
                ...
                final imageUrl = controller.getFeedImageUrl(item); // Get full URL
  ...
  Widget _buildLoadMoreIndicator(EvNewsFeedController controller) {
    return Padding(
      ...
      child: Center(
        child:
            controller.isLoading.value
                ...
                : Text('no_more_news'.tr, style: const TextStyle(color: Colors.grey)),
      ),
    );
  }

  Widget _buildErrorState(EvNewsFeedController controller) {
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          ElevatedButton(onPressed: controller.refreshData, child: Text('retry'.tr)),
        ],
      ),
    );
  }
...
class NewsDetailBottomSheet {
  static void show({
    ...
    required EvNewsFeedController controller,
  }) {
    final title = controller.getLocalizedTitle(item);
    final description = controller.getLocalizedDescription(item);
    final date = controller.formatDate(item.created);
