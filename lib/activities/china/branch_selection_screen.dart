...
class _BranchSelectionScreenState extends State<BranchSelectionScreen> {
  ...
  Future<void> _loadBranches() async {
    ...
    controller.selectedProvince.value = widget.province;

    // Clear branches if province changed
    if (controller.selectedProvince.value?.id != widget.province.id) {
      controller.branches.clear();
      controller.filteredBranches.clear();
      _searchController.clear();
    }

    // Load branches if empty
    if (controller.branches.isEmpty) {
      await controller.loadBranches(widget.province.id!.toString());
    }
  }
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                ElevatedButton(onPressed: _loadBranches, child: Text('retry'.tr)),
              ],
            ),
          );
        }
          ...
          children: [
            ...
            Expanded(
              child: RefreshIndicator(
                onRefresh: () => controller.loadBranches(widget.province.id!.toString()),
  ...
  Widget _buildBranchList() {
    if (controller.filteredBranches.isEmpty && controller.branchSearchText.value.isNotEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ...
            const SizedBox(height: 16),
            Text('no_branches_found'.tr, style: const TextStyle(fontSize: 16, color: Colors.grey)),
          ],
        ),
      );
    }
    ...
    return ListView.separated(
      ...
      separatorBuilder:
          (context, index) => Divider(height: 1, thickness: 0.5, color: Colors.grey[300]),
      itemBuilder: (context, index) {
        ...
        return Container(
          ...
          child: ListTile(
            ...
            leading: const Icon(Icons.business, color: Color(0xFFD35F27), size: 20),
            title: Text(
              branch.name ?? '',
              style: TextStyle(
                fontSize: 15,
