...
import 'package:express_vet/feature/home-dashboard/ev-charger/presentation/screen/ev_province_selection_screen.dart';
import 'package:express_vet/feature/home-dashboard/ev-charger/presentation/screen/search_ev_screen.dart';
...
class _EvChargerState extends State<EvCharger> {
  ...
  String _getLocalizedField({
    ...
    switch (currentLocale) {
      case 'km': // Khmer
        return khmerField?.trim().isNotEmpty == true
            ? khmerField!
            : englishField ?? '';
      case 'zh': // Chinese
        return chineseField?.trim().isNotEmpty == true
            ? chineseField!
            : englishField ?? '';
  ...
  Set<Marker> _buildMarkers() {
    final stationsToShow =
        _selectedStation != null ? [_selectedStation!] : _allStations;

    return stationsToShow
        .where(
          (s) =>
              double.tryParse(s.lats ?? '') != null &&
              double.tryParse(s.longs ?? '') != null,
        )
        .map(
          (s) => Marker(
                      ...
                      onTap: () async {
                        final selectedStation = await Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:
                                (context) =>
                                    SearchEvScreen(allStations: _allStations),
                          ),
                        );

                        if (selectedStation != null &&
                            selectedStation is BodyEV) {
            ...
            child: FloatingActionButton(
              ...
              onPressed: () async {
                final result = await Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => EvProvinceSelectionScreen(),
                  ),
                );
          ...
          children: [
            ...
            if (_selectedProvinceName != null) ...[
              ...
              TextButton(
                ...
                child: Text('see_all_provinces'.tr),
              ),
            ],
          ],
        ),
