...
class EvWalletController extends GetxController {
  ...
  Future<void> fetchWalletTransactions({bool loadMore = false}) async {
    try {
      ...
      if (response.body?.status == true) {
        ...
        hasMoreData.value =
            currentItemCount < totalTransactions.value && newGroups.isNotEmpty;
        ...
        hasError.value = true;
        errorMessage.value =
            response.body?.message ?? 'Failed to load transactions';
      }
    } catch (e) {
      hasError.value = true;
      errorMessage.value = e.toString();
  ...
  int _countAllTransactions() {
    return walletTransactions.fold(
      0,
  ...
  void applyFilter(String filter) {
    ...
    for (var group in walletTransactions) {
      ...
      switch (filter) {
        ...
        case 'expense':
          filteredGroupTransactions =
              transactions.where((t) => t.type == 2).toList();
          break;
        default: // 'all'
      ...
      if (filteredGroupTransactions.isNotEmpty) {
        filteredTransactions.add(
          Group(date: group.date, transactions: filteredGroupTransactions),
        );
      }
    }
  }
