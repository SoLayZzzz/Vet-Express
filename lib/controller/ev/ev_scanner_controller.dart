...
import '../../activities/ev/ev_charger_wallet_screen.dart';
import '../../api/ev.dart';
import '../../models/ev/ev_scan_qr_response.dart';
...
class EvScannerController extends GetxController {
  ...
  Future<void> confirmPayment() async {
    ...
    try {
      final response = await _apiService.eVConfirmPayment(Get.context!, transactionId);

      if (response.header?.result == true && response.header?.statusCode == 200) {
        if (response.body?.status == true && response.body?.message == 'success') {
          // Update wallet balance after charging
          if (transactionData.value != null && transactionData.value!.totalPrice != null) {
  ...
  void _showFullScreenPaymentSuccessDialog() {
    ...
    Get.dialog(
      _FullScreenPaymentSuccessDialog(
        data: data,
        onDone: _navigateToWalletScreen, // Updated to navigate to EvChargerScreen
        onScanAgain: () {
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Station Name
                      _buildDetailRow("Station Name", data?.stationName ?? "N/A"),
                      const Divider(height: 30),
                      ...
                      // Total kWh
                      _buildDetailRow("Total kWh", "${data?.totalKwh ?? 0} kWh"),
                      const Divider(height: 30),
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          if (data != null) ...[
                            _buildSummaryRow("Transaction ID", data?.transactionId ?? "N/A"),
                            const SizedBox(height: 12),
                            _buildSummaryRow("Station Name", data?.stationName ?? "N/A"),
                            const SizedBox(height: 12),
                            _buildSummaryRow(
                              "Amount",
                              "${data?.totalPrice?.toStringAsFixed(2) ?? "0.00"} KHR",
                            ...
                            const SizedBox(height: 12),
                            _buildSummaryRow("Energy", "${data?.totalKwh ?? 0} kWh"),
                            const SizedBox(height: 12),
