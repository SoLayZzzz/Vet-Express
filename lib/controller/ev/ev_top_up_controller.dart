...
import '../../activities/ev/ev_payment_screen.dart';
import '../../activities/ev/payment_success_screen.dart';
import '../../api/ev.dart';
...
class EvTopUpController extends GetxController {
  ...
  final List<double> predefinedAmounts = [5000, 10000, 20000, 50000, 100000, 200000];
  ...
  Future<void> performTopUp() async {
    ...
    try {
      ...
      final response = await _evApi.evWalletTopUp(Get.context!, amountInCents.toDouble());
      ...
      if (response.header?.statusCode == 200 && response.body?.status == true) {
        ...
        if (data?.deeplink != null &&
          ...
          Get.to(
            () => EvPaymentScreen(deepLink: data.deeplink!, checkoutQrUrl: data.checkoutQrUrl!),
          );
  ...
  Future<void> _checkPaymentStatus() async {
    ...
    try {
      final response = await _evApi.evPaymentStatus(Get.context!, currentTransactionId.value);
  ...
  void _closeAllPaymentScreens() {
    // Close payment screen if it's open
    if (Get.isDialogOpen == true) {
      Get.back();
    }

    // Close payment webview screen
    if (Get.currentRoute.contains('EvPaymentScreen')) {
      Get.back();
    }

    // Also close the top-up screen if it's still open
    if (Get.currentRoute.contains('EvTopUpScreen')) {
      Get.back();
    }
  }
